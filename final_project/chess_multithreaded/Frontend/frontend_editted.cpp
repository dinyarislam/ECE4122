...
{
    bool whiteTurn = true;
    bool playerColourWhite = true;


    /* Starting screen which gives the users the option to choose the:
        Difficulty level
        Board texture
        Piece texture
        Player colour (difficult?) Rotate the entire render window?
    */
    ...
    for (int i = 0; i < 64; i++)
    {
        ...
        // if (tempTileObj.currentPiece.getPosition().x == 2002) {
        // }
    ...
    //     if (tileObjs[xy].isPawn) {
    //         cout << tileObjs[xy].notation.c_str() << endl;
    //     }
    // }

    mte::Engine eng;

    // Setting the depth of the engine
    eng.depth = 4;
    ...
    while (window.isOpen())
    {
        ...
        if (!(whiteTurn && playerColourWhite)) {
            ...
            if (!checkMated) {
                ...
                // The piece being promoted is mv_txt[4].
                Tile* src;
                Tile* dstTile;
                ...
                if (shortCastle) {
                    ...
                    Sprite tmp;
                    tmp.setTexture(textureBlackBishop);
                    tmp.setScale(0.9, 1);
                    tmp.setPosition(20020.0f, 20020.0f);
            ...
            // Checking for check mate after the computer has moved.
            int matedOutput = eng.isMate(eng.cr);
            if (matedOutput == 3 || matedOutput == 4) {
                printf("CHECK MATED\n");
                checkMated = true;
            }
            else if (matedOutput == 1 || matedOutput == 2) {
                staleMated = true;
                printf("STALE MATED\n");
            }
        ...
        while (window.pollEvent(event))
        {
            ...
            if (event.type == sf::Event::MouseButtonPressed)
            {
                ...
                if (!checkMated && passThrough && !isPieceSelected) {
                    ...
                    if (!checkMated && !isPlayerColourPieceSelected) {
                        std::string moveNotation;

                        // Both long, short castle for white AND black.
                        ...
                        for (int j = 0; !illegalMove && j < 64; j++) {
                            if (tileObjs[j].piecePresent && (tileObjs[j].notation == selectedSquare)) {
                                ...
                                if (dstTile->currentPiece.getPosition().x == 20020.0f && dstTile->currentPiece.getPosition().y == 20020.0f) {
                                    ...
                                    dstTile->currentPiece.setPosition(2000.0f, 2000.0f);
                                }
                                ...
                                if (isPromotion) {
                                    ...
                                    if (isEnPassant) {
                                        ...
                                        for (int xy = 0; xy < 64; xy++) {
                                            if (tileObjs[xy].notation == srcTile->notation) {
                                                // Check the tile *below* / *above* it on the board
                                                if (!isCapture && tileObjs[xy + offset].piecePresent && tileObjs[xy + offset].isPawn && tileObjs[xy + offset].whitePiece != playerColourWhite) {
                                                    // is an en-passant
                                                    Sprite tmp;
        ...
        if (difficultySelected && Keyboard::isKeyPressed(Keyboard::Return))
        {
            ...
            if (textureSelected && pieceShadowSelected && !difficultySelected && difficultyLevel > 0)
            {
...
// Function that lets the engine make the move
// TerseOut returns the startPostendPos (characters 0, 1, 2, 3). The 4th char is the piece that the engine has chosen to promote the pawn to.